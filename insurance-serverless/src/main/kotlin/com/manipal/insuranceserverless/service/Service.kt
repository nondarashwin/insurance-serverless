package com.manipal.insuranceserverless.service
...
@Service
class Service {
    //final var mongoClient: com.mongodb.MongoClient =com.mongodb.MongoClient(MongoClientURI(""))
    ...
    fun charge(data: String, model: Model): ResponseEntity<String?> {
        dao = mongoTemplate?.let { Dao(it) }
    ...
    fun addPartner(data: String):String {
        if (mongoTemplate != null) {
            dao = mongoTemplate?.let { Dao(it) }
        }
    ...
    fun findFormConfig(category: String, product: String): List<Document> {
        ...
        query["category"] = category
        query["product"] = product
        return dao?.find("formConfig", query) ?: ArrayList<Document>()
